...
import { domainToSlug } from './domain-extractor';
 ...
 */
export function publishPublicationSourcePage(
  ...
  try {
    ...
    parsed.data.updated = params.finalizedAt;

    // Req 8.4: Append new source page slug to `sources` array
    if (Array.isArray(parsed.data.sources)) {
      if (!parsed.data.sources.includes(params.sourcePageSlug)) {
        parsed.data.sources.push(params.sourcePageSlug);
      }
    } else {
      parsed.data.sources = [params.sourcePageSlug];
    }
