...
describe('constructSourcePagePath', () => {
  describe('basic path construction', () => {
    ...
    it('constructs path from multi-word title', () => {
      const result = constructSourcePagePath('Understanding Reactive Programming in Angular', '2025-01-20');
      expect(result).toBe('wiki/sources/understanding-reactive-programming-in-angular-article-2025-01-20.md');
    });
  ...
  describe('error handling', () => {
    ...
    it('throws when finalizedAt is empty', () => {
      expect(() => constructSourcePagePath('Title', '')).toThrow('finalizedAt must be a valid date in YYYY-MM-DD format');
    });

    it('throws when finalizedAt is not YYYY-MM-DD format', () => {
      expect(() => constructSourcePagePath('Title', '2024/01/01')).toThrow('finalizedAt must be a valid date in YYYY-MM-DD format');
    });

    it('throws when finalizedAt is a partial date', () => {
      expect(() => constructSourcePagePath('Title', '2024-01')).toThrow('finalizedAt must be a valid date in YYYY-MM-DD format');
    });

    it('throws when finalizedAt has extra characters', () => {
      expect(() => constructSourcePagePath('Title', '2024-01-01T00:00:00')).toThrow('finalizedAt must be a valid date in YYYY-MM-DD format');
    });
  });
});
