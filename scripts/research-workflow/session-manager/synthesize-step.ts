...
export async function generateFindingsSummary(
  ...
  const analysisPath = join(sessionDir, 'article-analysis.md');
  ...
  const sessionContent = await readFile(sessionJsonPath, 'utf-8');
  ...
  const outputPath = join(sessionDir, 'findings-summary.md');
