...
import type { SessionJson, SessionState } from '../types/article-session';
 ...
 */
export async function pauseSession(sessionDir: string): Promise<SessionJson> {
 ...
 */
export async function resumeSession(sessionDir: string): Promise<ResumeResult> {
  const sessionJsonPath = path.join(sessionDir, 'session.json');
  const content = await fs.readFile(sessionJsonPath, 'utf-8');
  const session: SessionJson = JSON.parse(content);
  ...
  const artifactPaths = await discoverArtifacts(sessionDir);
 ...
 */
async function discoverArtifacts(sessionDir: string): Promise<string[]> {
  ...
  for (const artifact of knownArtifacts) {
    try {
      await fs.access(path.join(sessionDir, artifact));
      found.push(artifact);
    } catch {
