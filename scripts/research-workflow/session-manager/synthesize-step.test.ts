...
describe('generateFindingsSummary (integration)', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'synthesize-test-'));
  });
  ...
  it('reads article-analysis.md and session.json, writes findings-summary.md', async () => {
    ...
    await fs.writeFile(
      path.join(tmpDir, 'article-analysis.md'),
      SAMPLE_ANALYSIS,
      'utf-8'
    );
    await fs.writeFile(
    ...
    const content = await generateFindingsSummary(tmpDir);

    // Verify the file was written
    const written = await fs.readFile(
      path.join(tmpDir, 'findings-summary.md'),
      'utf-8'
    );
...
describe('updateFindingsSummary', () => {
  ...
  it('writes updated content to findings-summary.md', async () => {
    ...
    await updateFindingsSummary(tmpDir, updatedContent);
  ...
  it('overwrites existing content completely', async () => {
    ...
    await updateFindingsSummary(tmpDir, newContent);
...
describe('completeSynthesizeStep', () => {
  ...
  it('transitions session state from SYNTHESIZE to FINALIZE', async () => {
    ...
    await completeSynthesizeStep(tmpDir);

    const updated = JSON.parse(
      await fs.readFile(path.join(tmpDir, 'session.json'), 'utf-8')
    );
  ...
  it('throws when session is not in SYNTHESIZE state', async () => {
    ...
    await expect(completeSynthesizeStep(tmpDir)).rejects.toThrow();
  });
});
