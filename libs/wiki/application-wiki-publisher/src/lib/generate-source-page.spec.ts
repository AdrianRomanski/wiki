/**
 * Unit + property-based tests for normalized source page generation
 * Feature: article-research-session, article-author-source-discovery, scripts-migration-hexagonal
 *
 * Migrated from:
 * - scripts/research-workflow/wiki-publisher/generate-source-page.pbt.test.ts
 * - scripts/research-workflow/wiki-publisher/generate-source-page.test.ts
 *
 * `generateSourcePage` is pure (no I/O) and its signature is unchanged by the
 ...
 * - OLD "Property 9: Source page metadata includes author and publication source
 *   WikiLinks" + OLD "Property 11: Tag arrays contain required discovery tags"
 *   together map to THIS SPEC's **Property 8: Source page discovery metadata**
 *   (tagged below).
 *
...
const tagArbitrary = fc.stringMatching(/^[a-z][a-z0-9-]{0,19}$/).filter((s) => !s.endsWith('-'));
...
const authorWikiLinkArbitrary = fc
  .tuple(fc.stringMatching(/^[A-Z][a-z]{1,14}$/), fc.stringMatching(/^[A-Z][a-z]{1,14}$/))
...
// ============================================================================
// Feature: scripts-migration-hexagonal, Property 8: Source page discovery metadata
// (combines OLD Property 9 and OLD Property 11)
// ============================================================================

describe('Feature: scripts-migration-hexagonal, Property 8: Source page discovery metadata', () => {
  /**
   * Property 8: Source page discovery metadata
   *
   * For any source page generated with an author WikiLink and/or a
   * publication-source WikiLink, the Metadata section SHALL contain the
   * corresponding `[[WikiLink]]`, and the tags array SHALL contain the
   * author's kebab-case slug and/or the domain slug (dots→hyphens)
   * respectively.
   *
   * **Validates: Requirements 6.2**
   */
  ...
  it('metadata section contains [[publicationSourceWikiLink]] when publicationSourceWikiLink param is present', () => {
    fc.assert(
      fc.property(validParamsWithBothLinksArbitrary, (params) => {
        ...
        expect(metadataSection).toContain(`[[${params.publicationSourceWikiLink}]]`);
      }),
      { numRuns: 100 }
    );
  });
  ...
  it('tags array contains domain slug (dots→hyphens) when publicationSourceWikiLink is present', () => {
    fc.assert(
      fc.property(validParamsWithPubSourceOnlyArbitrary, (params) => {
        ...
        const expectedTag = params.publicationSourceWikiLink!.replace(/\./g, '-');
  ...
  it('tags array contains both author tag and domain tag when both params are present', () => {
    fc.assert(
      fc.property(validParamsWithBothLinksArbitrary, (params) => {
        ...
        const expectedDomainTag = params.publicationSourceWikiLink!.replace(/\./g, '-');
...
// ============================================================================
// Unit tests: generateSourcePage
// Migrated from scripts/research-workflow/wiki-publisher/generate-source-page.test.ts
// ============================================================================
...
describe('generateSourcePage', () => {
  ...
  describe('body sections (Req 7.2)', () => {
    ...
    it('includes a Key Points section with at least one bullet', () => {
      const result = generateSourcePage(
        createValidParams({ keyPoints: ['First point', 'Second point'] })
      );
    ...
    it('includes an Insights section with at least one takeaway', () => {
      const result = generateSourcePage(
        createValidParams({ insights: ['Key takeaway from the article.'] })
      );
    ...
    it('includes a Relevant Entities section', () => {
      const result = generateSourcePage(createValidParams({ entities: ['Angular Signals'] }));
      expect(result).toContain('## Relevant Entities');
    });

    it('includes a Relevant Concepts section', () => {
      const result = generateSourcePage(createValidParams({ concepts: ['Fine-Grained Reactivity'] }));
  ...
  describe('WikiLink syntax (Req 7.3)', () => {
    it('uses [[Page Title]] syntax for entity cross-references', () => {
      const result = generateSourcePage(createValidParams({ entities: ['Angular Signals', 'RxJS'] }));
    ...
    it('uses [[Page Title]] syntax for concept cross-references', () => {
      const result = generateSourcePage(
        createValidParams({ concepts: ['Fine-Grained Reactivity', 'Change Detection'] })
      );
    ...
    it('uses page title (not filename) in WikiLinks', () => {
      const result = generateSourcePage(
        createValidParams({ entities: ['Angular CDK'], concepts: ['Progressive Enhancement'] })
      );
  ...
  describe('validation', () => {
    it('throws when title is empty', () => {
      expect(() => generateSourcePage(createValidParams({ title: '' }))).toThrow(
    ...
    it('throws when tags array is empty', () => {
      expect(() => generateSourcePage(createValidParams({ tags: [] }))).toThrow(
        'tags must contain at least one element'
      );
    });

    it('throws when keyPoints array is empty', () => {
      expect(() => generateSourcePage(createValidParams({ keyPoints: [] }))).toThrow(
        'keyPoints must contain at least one element'
      );
    });

    it('throws when insights array is empty', () => {
      expect(() => generateSourcePage(createValidParams({ insights: [] }))).toThrow(
        'insights must contain at least one element'
      );
    });

    it('throws when created is not YYYY-MM-DD format', () => {
      expect(() => generateSourcePage(createValidParams({ created: '2024/06/01' }))).toThrow(
        'created must be a valid date in YYYY-MM-DD format'
      );
    });

    it('throws when updated is not YYYY-MM-DD format', () => {
      expect(() => generateSourcePage(createValidParams({ updated: 'June 1, 2024' }))).toThrow(
        'updated must be a valid date in YYYY-MM-DD format'
      );
    });
  });
  ...
  describe('author and publication source WikiLinks (Req 3.1, 3.2, 3.3, 3.4, 3.5)', () => {
    describe('with both authorWikiLink and publicationSourceWikiLink', () => {
      it('renders author as WikiLink in Metadata section', () => {
        const result = generateSourcePage(
          createValidParams({ authorWikiLink: 'Manfred Steyer', publicationSourceWikiLink: 'nx.dev' })
        );
    ...
    describe('with only authorWikiLink', () => {
      it('renders author as WikiLink in Metadata section', () => {
        const result = generateSourcePage(
          createValidParams({ authorWikiLink: 'Jane Doe', publicationSourceWikiLink: undefined })
        );
    ...
    describe('with only publicationSourceWikiLink', () => {
      ...
      it('renders publication source as WikiLink in Metadata section', () => {
        const result = generateSourcePage(
          createValidParams({ authorWikiLink: undefined, publicationSourceWikiLink: 'medium.com' })
        );
    ...
    describe('with neither authorWikiLink nor publicationSourceWikiLink', () => {
      it('renders author as Unknown when author param is also absent', () => {
        const result = generateSourcePage(
          createValidParams({ author: undefined, authorWikiLink: undefined, publicationSourceWikiLink: undefined })
        );
        expect(result).toContain('- **Author:** Unknown');
      });
      ...
      it('does not include publication source line in Metadata section', () => {
        const result = generateSourcePage(
          createValidParams({ authorWikiLink: undefined, publicationSourceWikiLink: undefined })
        );
        expect(result).not.toContain('**Publication Source:**');
      });
